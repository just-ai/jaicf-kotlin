...
class ConfirmToolCallTest: LLMScenarioTest(AgentWithConfirmation) {
    ...
    @Test
    fun `agent requires confirmation before calling tool`() = testWithLLM({model = "gpt-4o-mini"}) {
        send(user = "Send mail with text: '${sendMail.message}'", agent = "Asks user to confirm sending the message") callsTool sendMail
